...
# To make SURE we match the English Practice Notebook Sheet end:
# We will find "English Practice Notebook Sheet" and then find the NEXT `</section>`!
idx_start = content.find('English Practice Notebook Sheet')
if idx_start != -1:
    idx_end = content.find('</section>', idx_start)
    if idx_end != -1:
        # The section ends at `</section>`.
        # We want to insert `timoUiStr` right after the `</section>`.
        # Wait, `</section>` is at `idx_end`. We add `len('</section>')`.
        idx_insert = idx_end + len('</section>')
        
        content = content[:idx_insert] + "\n" + timoUiStr + "\n" + content[idx_insert:]
        print("Patched end successfully by inserting after English Practice Notebook Sheet's </section>!")
else:
    print("Failed to find English Practice Notebook Sheet")
