...
content = content.replace(
  ...
  `import { VOCABULARY_LIST, SENTENCE_LIST } from "@/data/englishData";\nimport { getTimoTest, getTimoTestCount, TimoRound } from "@/data/timoData";`
);
...
const handleStartTimoStr = `
  const handleStartTimo = () => {
    const data = getTimoTest(timoGrade, timoRound, timoTestIndex);
  ...
  const handleCheckTimo = async () => {
    ...
    try {
      ...
      if (typeof loadTimoHistoryList === 'function') {
        loadTimoHistoryList();
      }
    } catch (e) {
                                ...
                                {isTimoChecked ? (
                                  ...
                                  <div className={\`flex-1 flex flex-col gap-6 \${timoQuestions[activeTimoIndex]?.shake ? "animate-shake" : ""}\`}>
                                    ...
                                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-auto">
                                      {timoQuestions[activeTimoIndex]?.options?.map((opt: any, optIndex: number) => {
...
content = content.replace(
  `{englishQuestions.length > 0 && (\n                        <button\n                          onClick={() => setIsRecordingEnglish`,
  timoUiStr + `\n                      {englishQuestions.length > 0 && (\n                        <button\n                          onClick={() => setIsRecordingEnglish`
);

// Clear old duplicates
content = content.replace(/const loadTimoHistoryList = async \(\) => \{[\s\S]*?\};\n\n  const handleClearTimoHistory = async \(\) => \{[\s\S]*?\};\n/g, "");
